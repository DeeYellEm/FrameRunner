...
def init(main_batch, framerunner_label):
    ...
    print('== DLM: example:init()')

    # Set up the top label
    example_label = pyglet.text.Label(text="Example", x=200, y=300, color=(255, 255, 255, 255), anchor_x='center', batch=main_batch)
    framerunner_label.text = "Goombah!!"
    example_label.text = "Now Example"
    example_label.draw()
...
def timerFunc (dt):
    foo = 0
    #print('DLM: Example:timerFunc called.')
    # timerFunc Stuff
...
def alldone(dt):
    ...
    print('DLM: Example: All Done.  Returning Control.')
    pyglet.app.exit()
